...
curl --request PUT -H "PRIVATE-TOKEN: $GITLAB_TOKEN" "http://$GITLAB_NEW_FQDN/api/v4/application/settings?allow_local_requests_from_hooks_and_services=true"

# Clone the wordpress-develop repository from GitHub to the GitLab internal server
curl --header "PRIVATE-TOKEN: $GITLAB_TOKEN" -X POST "http://$GITLAB_NEW_FQDN/api/v4/projects?name=wordpress-develop&issues_enabled=false&import_url=git://github.com/WordPress/wordpress-develop.git"

# Create the Jenkins web hook for the Build-WordPress-Job pipeline
echo -e "\nSetting up web hook in wordpress-develop repository"
curl -H "PRIVATE-TOKEN: $GITLAB_TOKEN" -H 'Content-Type: application/json' -X POST "http://$GITLAB_NEW_FQDN/api/v4/projects/root%2Fwordpress-develop/hooks" -d '{
...
curl --header "PRIVATE-TOKEN: $GITLAB_TOKEN" -X POST "http://$GITLAB_NEW_FQDN/api/v4/projects?name=wordpress-cd&issues_enabled=false&default_branch=4.9"
...
mkdir wordpress-cd
cd wordpress-cd
git clone "http://oauth2:$GITLAB_TOKEN@$GITLAB_NEW_FQDN/root/wordpress-cd.git"
